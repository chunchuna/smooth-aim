...
// V12 raw output [1, 4+nc, num_anchors], anchors are contiguous within each channel.
// Three paths: AVX2 (8/iter) -> SSE4.1 (4/iter) -> Scalar.
void DMLYoloV12Detector::GenerateProposals(float* output, std::vector<DMLObject>& proposals, float conf) {
    ...
    if (SIMDSupport::avx2Supported()) {
        ...
        for (; d < num_detections; ++d) {
            float maxScore = -FLT_MAX; int classId = -1;
            for (int c = 0; c < numClasses; ++c) {
